...
if(count($routesArray) == 0){
	...
	//echo '<pre>'; print_r($routesArray[2]);echo '</pre>';
	...
	if(count($routesArray) == 2 &&
	   ...
	   $_SERVER["REQUEST_METHOD"] == "GET"){
		...
		}else if(isset($_GET["rel"]) && isset($_GET["type"]) && explode("?", $routesArray[2])[0] == "relations" && !isset($_GET["linkTo"]) && !isset($_GET["equalTo"]) ){
		...
		=============================================*/
			...
			if( explode("?", $routesArray[2])[0] == "relations" && isset($_GET["rel"]) && isset($_GET["type"])){
				...
				$response = new GetController();
				$response -> getSearchData(explode("?", $routesArray[2])[0], $_GET["linkTo"], $_GET["search"], $orderBy, $orderMode, $startAt, $endAt);
		...
		if(isset($_POST)){
			...
			if($count > 0){
				...
				if(isset($_GET["register"]) && $_GET["register"] == true){
					...
					$response -> postRegister(explode("?", $routesArray[2])[0], $_POST);
				...
				=============================================*/	
					...
					$response = new PostController();
					$response -> postLogin(explode("?", $routesArray[2])[0], $_POST);
